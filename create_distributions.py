...
def create_macos_package():
    ...
    # Copy plugins
    plugins_dir = package_dir / "SketchyBar_Plugins"
    plugins_dir.mkdir()
    if Path("plugins").exists():
        shutil.copytree("plugins", plugins_dir / "plugins")
    
    # Copy configuration example
    if Path("example_sketchybarrc").exists():
        shutil.copy2("example_sketchybarrc", plugins_dir)
    
    # Copy setup script
    if Path("setup_timetracker.sh").exists():
        shutil.copy2("setup_timetracker.sh", package_dir)
        os.chmod(package_dir / "setup_timetracker.sh", 0o755)
...
Contents:
- IACLS Time Tracker.app - Main application
- SketchyBar_Plugins/ - SketchyBar integration files
- setup_timetracker.sh - Automated setup script
- Documentation/ - Installation guides and documentation

Quick Start:
1. Read Documentation/macOS_Installation_Guide.pdf
2. Run setup_timetracker.sh for automated setup
3. Launch IACLS Time Tracker.app
