...
update_sketchybar() {
    ...
    if [ -n "$current_project" ]; then
        # Tracking - black text/icon on green background
        local elapsed=$(get_elapsed_time)
        sketchybar --set time_tracker icon="⏱" label="$current_project ($elapsed)" \
                   icon.color=0xff000000 label.color=0xff000000 \
                   background.color=0xff008000 background.corner_radius=5 background.height=25
    else
        # Not tracking - red icon only, no text
        sketchybar --set time_tracker icon="⏱" label="" \
                   icon.color=0xffff0000 label.color=0xffff0000 \
                   background.color=0x00000000 background.corner_radius=0 background.height=0
    fi
}
